...
| swizzle mode | type num | blk_type         | sw_type |
| ------------ | -------- | ---------------- | ------- |
| SW_LINEAR    | 0        | SZ_LIN (SZ_128B) | SW_L    |
| SW\_256B_2D  | 1        | SZ_256B          | SW_D_2D |
| SW\_4KB_2D   | 2        | SZ_4KB           | SW_D_2D |
| SW\_64KB_2D  | 3        | SZ_64KB          | SW_D_2D |
| SW\_256KB_2D | 4        | SZ_256KB         | SW_D_2D |
| SW\_4KB_3D   | 5        | SZ_4K            | SW_S_3D |
| SW\_64KB_3D  | 6        | SZ_64KB          | SW_S_3D |
| SW\_256KB_3D | 7        | SZ_256KB         | SW_S_3D |
...
swizzle_bits_256B =baseAddr256B_in & (blk_bit_mask_128B >> 1);

baseAddr256B_out = {baseAddr256B_in[ADDR_WIDTH_256B-1:BLK_256B_WIDTH], (baseAddr256B_in[BLK_256B_WIDTH-1:0] & ~blk_bit_mask_128B >> 1)};

mipoffset_BaseAddr256B_out = baseAddr256B_out + mip_offset_b_in;
...
input log2_blk_width;

input log2_blk_width_slice;

input log2_blk_height;

input log2_blk_depth;
...
//Use log2_blk_width_slice because slice is still 256B aligned
...
micro_offset_linear = x_in[6:0] << l2_eb_in;
...
blk_offset_final = (linear) ? {1'b0, micro_offset_linear} : blk_offset;

swizzle_bits = blk_offset[DATA_BLK_OFFSET_WIDTH-1:8] & ms_mask_256B[DATA_BLK_OFFSET_WIDTH-8-1:0];
